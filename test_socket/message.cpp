...
int Socket::send(std::string msg, double delay) const
{
  if(delay == 0.0)
...
int Socket :: recv_from(char* buf, int len, std::string addr, int port, double delay)
{
  _addr.sin_family = _af;
  _addr.sin_addr.s_addr = ::inet_addr(addr.c_str());
  _addr.sin_port = ::htons(port);
...
std::string Socket :: recv_from(std::string addr, int port, double delay)
{
  _addr.sin_family = _af;
  _addr.sin_addr.s_addr = ::inet_addr(addr.c_str());
  _addr.sin_port = ::htons(port);
