...
namespace Werewolf{
  ...
  public:
    ...
    int send(std::string msg, double delay = 0.0);//功能和返回值与原send函数相同
    ...
    int recv_from(char *buf, int len,  std::string addr, int port);//功能和返回值同原函数
    std::string recv_from(std::string addr, int port);
